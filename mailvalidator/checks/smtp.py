"""SMTP diagnostics: connectivity, banner, PTR, EHLO, ESMTP extensions, open relay,
STARTTLS, and deep TLS inspection.

This module targets **external-facing MX servers** that accept inbound mail on
port 25 (RFC 5321 §2.1, §4.5.3.2).  Port 587 is the *submission* port (RFC 6409)
and requires AUTH — it is a different service and these checks are not meaningful
against it.
...
- Connect latency
- 220 banner FQDN validation (RFC 5321 §4.1.3): the greeting MUST name the server
- Reverse DNS (PTR) for the server IP
- EHLO acceptance and domain line validation (RFC 5321 §4.1.1.1)
- ESMTP extension advertisement: STARTTLS (RFC 3207), SIZE (RFC 1870),
  PIPELINING (RFC 2920), 8BITMIME (RFC 6152), SMTPUTF8 (RFC 6531)
